...
with st.sidebar:
    ...
    subtema = st.selectbox("Subtema", options=[
        "Todos", "anatomia_fetal", "doppler", "embarazo_multiple",
        "formacion", "neurosonograma", "primer_trimestre",
        "procedimientos", "segundo_trimestre",
    ])

    tipo_doc = st.selectbox("Tipo de documento", options=[
        "Todos", "guia_clinica", "protocolo", "manual",
    ])
...
if st.button("Consultar", type="primary") and pregunta.strip():
    ...
    # --- Respuesta ---
    intento = resultado.get("intento", 1)
    relevancia = resultado.get("relevancia", "SÍ")

    if relevancia == "NO" and intento == 2:
        # El agente no encontró contexto relevante en ningún intento
        st.warning("⚠️ Tu pregunta no corresponde al contenido de las guías disponibles.")
        st.markdown(resultado["respuesta"])

        if resultado.get("pregunta_reformulada"):
            st.markdown("#### 💡 Intentá con esta pregunta:")
            st.code(resultado["pregunta_reformulada"], language=None)
        ...
        if resultado.get("pregunta_reformulada"):
            with st.expander("🔄 Pregunta reformulada"):
                st.code(resultado["pregunta_reformulada"], language=None)
